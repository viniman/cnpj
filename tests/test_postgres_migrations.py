...
class PostgresMigrationsTest(unittest.TestCase):
    ...
    def test_apply_script_tracks_migrations_by_version_and_checksum(self):
        ...
        self.assertIn("$existingChecksumResult", script)
        self.assertIn("$null -eq $existingChecksumResult", script)
    ...
    def test_snapshot_import_script_loops_recognized_files(self):
        ...
        self.assertIn("check_receita_staging_preflight.ps1", script)
        self.assertIn("Preflight de carga completa falhou", script)
        self.assertIn("Format-Percent", script)
        self.assertIn("zip_size_bytes", script)
    ...
    def test_preflight_script_wraps_python_planner_and_docker_check(self):
        ...
        self.assertIn("$preflightExitCode", script)
        self.assertIn("Aplicacao de migrations falhou", script)
        self.assertIn("--free-bytes", script)
        self.assertIn("--disk-multiplier", script)
